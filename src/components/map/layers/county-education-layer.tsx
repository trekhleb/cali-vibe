...
const FILL_LAYER_ID = "county-education-fill";
...
export type EducationMetric = "bachPlus" | "hsPlus" | "gradPlus";

export const EDUCATION_LABELS: Record<EducationMetric, string> = {
  bachPlus: "Bachelor's Degree or Higher",
  hsPlus: "High School Diploma or Higher",
  gradPlus: "Graduate Degree or Higher",
};
...
function buildFillColor(metric: EducationMetric): ExpressionSpecification {
  ...
  return [
    "interpolate", ["linear"], educationExpr(metric),
    ...scale.stops.flat(),
...
interface CountyEducationLayerProps {
...
export default function CountyEducationLayer({ educationMetric, overlayOffset = 0, selectName = null, onToggleFavorite, onViewDetail, isFavorite }: CountyEducationLayerProps) {
  ...
  const highlightLineLayer: LineLayerSpecification = {
    id: "county-education-borders-highlight",
    type: "line",
    source: SOURCE_ID,
    paint: {
  ...
  return (
    <>
      ...
      <Source id={LABEL_SOURCE_ID} type="geojson" data={LABELS_URL}>
        <Layer {...labelLayer} id="county-education-labels-dim" filter={labelDimFilter} />
        <Layer
          {...labelLayer}
          id="county-education-labels-highlight"
