...
type SortKey = "name" | EducationMetric;
...
interface EducationTableModalProps {
  open: boolean;
  onClose: () => void;
  dataUrl: string;
  title: string;
...
const COLUMNS: { key: EducationMetric; short: string }[] = [
  { key: "bachPlus", short: "Bachelor's+" },
  { key: "hsPlus", short: "HS+" },
  { key: "gradPlus", short: "Graduate+" },
];

export default function EducationTableModal({
  open,
  onClose,
  dataUrl,
  title,
  nameLabel,
  activeEducationMetric,
  onSelectName,
}: EducationTableModalProps) {
  const [rows, setRows] = useState<EducationRow[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [sortKey, setSortKey] = useState<SortKey>(activeEducationMetric);
  const [sortDir, setSortDir] = useState<SortDir>("desc");

  useEffect(() => {
    setSortKey(activeEducationMetric);
    setSortDir("desc");
  }, [activeEducationMetric]);
  ...
  useEffect(() => {
    ...
    fetch(dataUrl)
      ...
      .then((gj) => {
        if (cancelled) return;
        const parsed: EducationRow[] = [];
        for (const feat of gj.features) {
          const { name, education } = feat.properties;
          if (name && education) {
            parsed.push({
              name,
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      const av = a.education[sortKey] ?? 0;
      const bv = b.education[sortKey] ?? 0;
            ...
            <tr>
              ...
              {COLUMNS.map((col) => (
                <th
                  ...
                  title={EDUCATION_LABELS[col.key]}
                >
          ...
          <tbody>
            {sorted.map((row, i) => (
              ...
              >
                ...
                {COLUMNS.map((col) => {
                  const val = row.education[col.key] as number | null;
                  return (
                    <td
                      key={col.key}
