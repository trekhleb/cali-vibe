#!/usr/bin/env node
/**
 * Build BART transit data as GeoJSON files (routes + stops).
 *
 * Data source: BART GTFS (bart.gov/schedules/developers/gtfs)
 * License: Public, free for developer use
 *
 * Outputs:
 *   public/data/transit/bart-routes.geojson   (LineString per route)
 *   public/data/transit/bart-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-bart.mjs
 */
...
const TMP_DIR = join(OUT_DIR, ".bart-gtfs-tmp");
const GTFS_URL = "https://www.bart.gov/dev/schedules/google_transit.zip";

// BART line colors — override GTFS where needed for map readability
const BART_COLORS = {
  "default": "#000000",
};
...
function downloadGTFS() {
  console.log("Downloading BART GTFS...");
  if (!existsSync(TMP_DIR)) mkdirSync(TMP_DIR, { recursive: true });

  const zipFile = join(TMP_DIR, "bart.zip");
...
function buildRoutes() {
  ...
  for (const r of routes) {
    routeMap[r.route_id] = {
      ...
      color: r.route_color ? `#${r.route_color}` : BART_COLORS.default,
  ...
  // Sort points and build features — deduplicate by color (each BART line
  // appears as two routes, one per direction; keep the longer shape per color)
  const features = [];
  const seenColors = {};
  ...
  for (const [shapeId, points] of Object.entries(shapePoints)) {
    ...
    const route = routeMap[routeId] || { name: routeId, color: BART_COLORS.default };
    const colorKey = route.color;

    if (seenColors[colorKey]) {
      const displayColor = COLOR_OVERRIDES[colorKey] || colorKey;
      const existing = features.find(f => f.properties.color === displayColor);
      if (existing && existing.geometry.coordinates.length >= points.length) continue;
      const idx = features.findIndex(f => f.properties.color === displayColor);
    ...
    features.push({
      ...
      properties: {
        ...
        color: COLOR_OVERRIDES[route.color] || route.color,
...
function buildStops() {
  ...
  const routeColors = {};
  for (const r of routes) {
    routeColors[r.route_id] = r.route_color ? `#${r.route_color}` : BART_COLORS.default;
  }
  ...
  // Filter to stations only (location_type=1, or location_type=0 without parent)
  // Exclude entrances (location_type=2), generic nodes (3), boarding areas (4)
  const features = [];
  ...
  for (const s of stops) {
    ...
    if (locationType >= 2) continue; // skip entrances, nodes, boarding areas
    if (locationType === 0 && s.parent_station) continue; // skip child platforms
    ...
    const colors = [...new Set([...routeIds].map(rid => {
      const raw = routeColors[rid] || BART_COLORS.default;
      return COLOR_OVERRIDES[raw] || raw;
    }))];
    ...
    features.push({
      ...
      properties: {
        ...
        system: "bart",
        routes: [...routeIds],
        colors: colors.length > 0 ? colors : [BART_COLORS.default],
      },
    });
  }
...
//
// Where multiple BART lines share the same physical track, shift each line's
// geometry perpendicular to the track direction so all lines are visible
// side-by-side instead of stacking on top of each other.
//
// Key design decisions:
//   1. Normalize route directions (all go west→east) so tangent vectors align
//      in shared sections — prevents opposite perpendiculars for opposite-
//      direction routes.
//   2. Propagate perpendicular direction sequentially along each route (dot-
//      product check) instead of per-point global normalization — prevents
//      sudden flips at curves where the track passes through a critical angle.

const OFFSET_SPACING_M = 150; // meters between adjacent overlapping lines
const OFFSET_GRID_DEG = 0.001; // grid cell size for overlap detection (~111 m)
const OFFSET_SMOOTH_WINDOW = 10; // moving-average window per pass (applied 3× for Gaussian-like shape)
const OFFSET_SMOOTH_PASSES = 3; // multi-pass smoothing: bell-shaped transition instead of linear ramp
...
function offsetRoutes(routesGeoJSON) {
  ...
  // Normalize all route directions so they go generally west→east
  // (or south→north as tiebreaker). This ensures routes sharing the same
  // physical track have aligned tangent vectors.
  const normCoords = features.map((f) =>
    normalizeDirection(f.geometry.coordinates),
  );

  // Spatial index: grid cell key → set of route indices
  const grid = {};
  ...
  // Stable ordering by color hex so each line fans to a consistent side
  ...
  // Pre-compute sequentially-consistent perpendicular directions per route
  const allPerps = normCoords.map((coords) => computePerps(coords));

  // Build reference perpendicular per grid cell (from lowest-ranked route)
  // so that overlapping routes sharing track use a consistent perpendicular
  // direction — prevents opposite-direction routes from offsetting to the
  // same side (e.g. orange & red lines in the Richmond branch).
  const cellRefPerp = {};
  for (let ri = 0; ri < features.length; ri++) {
  ...
  const offsetFeatures = features.map((feature, ri) => {
    ...
    // Raw perpendicular offset (in meters) at every point
    const rawOffsets = coords.map(([lng, lat], p) => {
      ...
      // Align perpendicular with the reference route: average the reference
      // perpendicular across neighbor cells for stability (prevents zigzag
      // from single-cell noise at curves).
      let refPx = 0, refPy = 0;
      let refFound = false;
      for (const key of oGridNeighborKeys(lng, lat)) {
    ...
    // Stabilize offset signs: at curves the perpendicular flip decision can
    // oscillate, producing isolated sign flips that cause zigzag artifacts.
    // A majority filter over a window ensures contiguous segments keep a
    // consistent sign.
    const stableOffsets = stabilizeSigns(rawOffsets, OFFSET_SMOOTH_WINDOW);

    // Multi-pass smooth for gradual fan-in / fan-out at branching points.
    // Multiple passes of a box filter approximate a Gaussian: the transition
    // stays flat in shared sections and bends quickly (but smoothly) at branches.
    ...
    // Apply pre-computed perpendicular shift to each coordinate
...
function computePerps(coords) {
  ...
  for (let p = 0; p < coords.length; p++) {
    ...
    if (p > 0) {
      // Keep aligned with previous point's perpendicular (dot-product check)
      ...
      // Seed: prefer the perpendicular that points "north" or "east"
 ...
 * that the majority of neighbors within the window share.  This eliminates
 * isolated sign flips caused by perpendicular alignment noise at curves.
 */
...
function main() {
  console.log("Building BART transit data\n");
  ...
  const routesFile = join(OUT_DIR, "bart-routes.geojson");
  const stopsFile = join(OUT_DIR, "bart-stops.geojson");
