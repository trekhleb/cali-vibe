#!/usr/bin/env node
/**
 * Build VTA Light Rail transit data as GeoJSON files.
 *
 * Data source: VTA GTFS
 * License: Public
 *
 * Outputs:
 *   public/data/transit/vta-routes.geojson   (LineString per route)
 *   public/data/transit/vta-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-vta.mjs
 */
...
const SYSTEM_ID = "vta";
const SYSTEM_LABEL = "VTA Light Rail";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://gtfs.vta.org/gtfs_vta.zip";

const DEFAULT_COLOR = "#0072BC";

// VTA light rail route_type = 0; filter by route_type to skip buses
...
function buildStops() {
  ...
  for (const s of stops) {
    ...
    if (routeIds.size === 0) continue; // skip bus-only stops
