#!/usr/bin/env node
/**
 * Build Sacramento RT Light Rail transit data as GeoJSON files.
 *
 * Data source: SacRT GTFS (filtered for light rail route_type=0)
 * License: Public
 *
 * Outputs:
 *   public/data/transit/sacrt-routes.geojson   (LineString per route)
 *   public/data/transit/sacrt-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-sacrt.mjs
 */
...
const SYSTEM_ID = "sacrt";
const SYSTEM_LABEL = "Sacramento RT";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://www.sacrt.com/wp-content/uploads/google_transit.zip";

// Light rail is route_type=0 in the SacRT feed
const LIGHT_RAIL_ROUTE_TYPE = "0";

const DEFAULT_COLOR = "#0000FF";

const COLOR_OVERRIDES = {
  "#EED211": "#C4A600", // Gold line: brighten the pale yellow for better map visibility
};
...
function buildRoutes() {
  ...
  console.log(`  Found ${validRouteIds.size} light rail routes: ${[...validRouteIds].join(", ")}`);
...
function buildStops() {
  ...
  for (const s of stops) {
    ...
    // SacRT stop names are uppercase — clean up then convert to title case
    const name = (s.stop_name || s.stop_id)
      .replace(/\s*\((NB|SB|EB|WB)\)\s*$/i, "")
      .replace(/\s+Station$/i, "")
