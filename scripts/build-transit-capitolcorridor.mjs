#!/usr/bin/env node
/**
 * Build Capitol Corridor transit data as GeoJSON files.
 *
 * Data source: Capitol Corridor GTFS
 * License: Public
 *
 * Outputs:
 *   public/data/transit/capitolcorridor-routes.geojson   (LineString per route)
 *   public/data/transit/capitolcorridor-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-capitolcorridor.mjs
 */
...
const SYSTEM_ID = "capitolcorridor";
const SYSTEM_LABEL = "Capitol Corridor";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://www.capitolcorridor.org/googletransit/GTFS.zip";

const DEFAULT_COLOR = "#1C4E8A";
