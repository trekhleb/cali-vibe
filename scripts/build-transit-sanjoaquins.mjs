#!/usr/bin/env node
/**
 * Build San Joaquins (Gold Runner) transit data as GeoJSON files.
 *
 * Data source: Amtrak San Joaquins GTFS (via Trillium)
 * License: Public
 *
 * Outputs:
 *   public/data/transit/sanjoaquins-routes.geojson   (LineString per route)
 *   public/data/transit/sanjoaquins-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-sanjoaquins.mjs
 */
...
const SYSTEM_ID = "sanjoaquins";
const SYSTEM_LABEL = "San Joaquins (Gold Runner)";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://data.trilliumtransit.com/gtfs/sanjoaquins-ca-us/sanjoaquins-ca-us.zip";

const DEFAULT_COLOR = "#1A6B8A";

const COLOR_OVERRIDES = {
  "#43a7c6": "#1A6B8A", // Replace pale GTFS teal with a more vibrant one
};
...
function buildStops() {
  ...
  for (const s of stops) {
    ...
    // Clean station names: "City, CA Amtrak Station" → "City"
    const name = (s.stop_name || s.stop_id)
      .replace(/,\s*CA\s+.*$/i, "")
      .replace(/\s*\(.*?\)\s*$/i, "")
      .replace(/\s+Amtrak\s+Station$/i, "")
      .replace(/\s+Station$/i, "")
      .replace(/\s+BART\s+Station$/i, "")
      .replace(/\s+-\s+/g, " — ");
