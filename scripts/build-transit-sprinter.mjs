#!/usr/bin/env node
/**
 * Build SPRINTER light rail transit data as GeoJSON files.
 *
 * Data source: NCTD GTFS (filtered for SPRINTER light rail)
 * License: Public
 *
 * Outputs:
 *   public/data/transit/sprinter-routes.geojson   (LineString per route)
 *   public/data/transit/sprinter-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-sprinter.mjs
 */
...
const SYSTEM_ID = "sprinter";
const SYSTEM_LABEL = "SPRINTER";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://lfportal.nctd.org/staticGTFS/google_transit.zip";

// SPRINTER is route_type=0 (tram/light rail) in the NCTD feed
const SPRINTER_ROUTE_TYPE = "0";

const DEFAULT_COLOR = "#E31837";
...
function downloadGTFS() {
  console.log(`Downloading ${SYSTEM_LABEL} GTFS (from NCTD feed)...`);
...
function buildRoutes() {
  ...
  // Map route_id → route info (only SPRINTER light rail)
  const routeMap = {};
  const validRouteIds = new Set();
  for (const r of routes) {
    if (r.route_type !== SPRINTER_ROUTE_TYPE) continue;
  ...
  console.log(`  Found ${validRouteIds.size} SPRINTER routes: ${[...validRouteIds].join(", ")}`);
...
function buildStops() {
  ...
  // Collect valid SPRINTER route IDs
  const validRouteIds = new Set();
  for (const r of routes) {
    if (r.route_type === SPRINTER_ROUTE_TYPE) validRouteIds.add(r.route_id);
  }
