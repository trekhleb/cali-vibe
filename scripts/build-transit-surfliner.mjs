#!/usr/bin/env node
/**
 * Build Pacific Surfliner transit data as GeoJSON files.
 *
 * Data source: Amtrak GTFS (filtered for route_id 78 — Pacific Surfliner)
 * License: Public
 *
 * Outputs:
 *   public/data/transit/surfliner-routes.geojson   (LineString per route)
 *   public/data/transit/surfliner-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-surfliner.mjs
 */
...
const SYSTEM_ID = "surfliner";
const SYSTEM_LABEL = "Pacific Surfliner";
...
const VALID_ROUTE_IDS = new Set(["78"]); // Pacific Surfliner

const DEFAULT_COLOR = "#1B5BA3";

const COLOR_OVERRIDES = {
  "#CAE4F1": "#1B5BA3", // Replace the pale GTFS blue with a vibrant one
};

// ── Download & extract GTFS ──

function downloadGTFS() {
  console.log(`Downloading ${SYSTEM_LABEL} GTFS (from Amtrak feed)...`);
...
function buildRoutes() {
  ...
  // Map route_id → route info (only Pacific Surfliner)
  const routeMap = {};
...
function buildStops() {
  ...
  for (const s of stops) {
    ...
    const name = (s.stop_name || s.stop_id)
      .replace(/\s*\(Amtrak\)\s*/i, "")
      .replace(/\s+Station$/i, "")
      .replace(/\s+-?\s*Amtrak$/i, "")
      .replace(/\s+Amtrak$/i, "");
