#!/usr/bin/env node
/**
 * Build COASTER commuter rail transit data as GeoJSON files.
 *
 * Data source: NCTD GTFS (filtered for COASTER commuter rail)
 * License: Public
 *
 * Outputs:
 *   public/data/transit/coaster-routes.geojson   (LineString per route)
 *   public/data/transit/coaster-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-coaster.mjs
 */
...
const SYSTEM_ID = "coaster";
const SYSTEM_LABEL = "COASTER";
...
// COASTER is route_type=2 (commuter rail) in the NCTD feed
const COASTER_ROUTE_TYPE = "2";

const DEFAULT_COLOR = "#00459D";
...
function buildRoutes() {
  ...
  // Map route_id → route info (only COASTER commuter rail)
  const routeMap = {};
  const validRouteIds = new Set();
  for (const r of routes) {
    if (r.route_type !== COASTER_ROUTE_TYPE) continue;
  ...
  console.log(`  Found ${validRouteIds.size} COASTER routes: ${[...validRouteIds].join(", ")}`);
...
function buildStops() {
  ...
  // Collect valid COASTER route IDs
  const validRouteIds = new Set();
  for (const r of routes) {
    if (r.route_type === COASTER_ROUTE_TYPE) validRouteIds.add(r.route_id);
  }
