#!/usr/bin/env node
/**
 * Build SMART (Sonoma-Marin Area Rail Transit) transit data as GeoJSON files.
 *
 * Data source: SMART GTFS via Trillium Transit
 * License: Public
 *
 * Outputs:
 *   public/data/transit/smart-routes.geojson   (LineString per route)
 *   public/data/transit/smart-stops.geojson    (Point per station)
 *
 * Usage: node scripts/build-transit-smart.mjs
 */
...
const SYSTEM_ID = "smart";
const SYSTEM_LABEL = "SMART";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);
const GTFS_URL = "https://data.trilliumtransit.com/gtfs/smart-ca-us/smart-ca-us.zip";

const DEFAULT_COLOR = "#4d6d51";

// Override the muted GTFS green with a brighter version for map visibility
const COLOR_OVERRIDES = {
  "#4d6d51": "#2E8B57", // sea green — vibrant but still green
};
...
function buildRoutes() {
  ...
  for (const r of routes) {
    const rawColor = r.route_color ? `#${r.route_color}` : DEFAULT_COLOR;
    routeMap[r.route_id] = {
...
function buildStops() {
  ...
  for (const s of stops) {
    ...
    const name = (s.stop_name || s.stop_id).replace(/^SMART\s+/i, "");
    ...
    if (routeIds.size === 0) continue; // skip stops with no route associations
