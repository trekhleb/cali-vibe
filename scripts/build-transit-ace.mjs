#!/usr/bin/env node
/**
 * Build ACE (Altamont Corridor Express) transit data as GeoJSON files.
 *
 * Data source: 511.org GTFS (requires SF_BAY_511_TOKEN env variable)
 * License: 511.org Data Agreement
 *
 * Outputs:
 *   public/data/transit/ace-routes.geojson   (LineString per route)
 *   public/data/transit/ace-stops.geojson    (Point per station)
 *
 * Usage: SF_BAY_511_TOKEN=xxx node scripts/build-transit-ace.mjs
...
const SYSTEM_ID = "ace";
const SYSTEM_LABEL = "ACE (Altamont Corridor Express)";
const OUT_DIR = new URL("../public/data/transit/", import.meta.url).pathname;
const TMP_DIR = join(OUT_DIR, `.${SYSTEM_ID}-gtfs-tmp`);

const API_TOKEN = process.env.SF_BAY_511_TOKEN;
if (!API_TOKEN) {
  console.error("Error: SF_BAY_511_TOKEN env variable is required.");
  console.error("Sign up at https://511.org/open-data/token");
  process.exit(1);
}

const GTFS_URL = `http://api.511.org/transit/datafeeds?operator_id=CE&api_key=${API_TOKEN}`;

const DEFAULT_COLOR = "#77297D";
